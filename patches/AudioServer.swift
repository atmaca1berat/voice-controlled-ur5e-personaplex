...
public struct AudioServer {
    ...
    public init(host: String = "127.0.0.1", port: Int = 8080, mode: String = "all", preload: Bool = false) {
    ...
    public func run() async throws {
        ...
        let wsConfig = WebSocketServerConfiguration(maxFrameSize: 1 << 24)
    ...
    func buildRouter() -> Router<BasicRequestContext> {
        ...
        if serverMode == "all" || serverMode == "asr" {
        ...
        if serverMode == "all" || serverMode == "pp" {
        router.post("/respond") { request, _ in
            ...
            let systemPrompt = params.string("system_prompt")
            ...
            let result: (audio: [Float], textTokens: [Int32])
...
private final class RealtimeSession {
    ...
    var mode: String = "transcribe_only"
    ...
    for try await message in inbound.messages(maxSize: 50 * 1024 * 1024) {
        ...
        case "session.update":
            if let sessionConfig = json["session"] as? [String: Any] {
                ...
                if let mode = sessionConfig["mode"] as? String {
                    session.mode = mode
                }
                if let prompt = sessionConfig["system_prompt"] as? String {
                    session.systemPrompt = prompt
                }
                if let voice = sessionConfig["voice"] as? String {
                    session.voice = voice
                }
            }
            try await outbound.write(.text(formatJSON([
        ...
        case "input_audio_buffer.commit":
            ...
            if session.mode == "personaplex_full_duplex" {
                let audio24k = resample(floats, from: session.inputSampleRate, to: 24000)
                let ppModel = try await state.loadPersonaPlex()
                let voiceEnum = PersonaPlexVoice(rawValue: session.voice) ?? .NATM0
                var totalSamples = 0
                var agentTranscript = ""

                let ppResult: (audio: [Float], textTokens: [Int32])
                if let prompt = session.systemPrompt {
                    ppResult = ppModel.respond(
                        userAudio: audio24k,
                        voice: voiceEnum,
                        systemPrompt: prompt,
                        maxSteps: 60)
                ...
                if !ppResult.audio.isEmpty {
                    totalSamples = ppResult.audio.count
                    let pcm = floatToPCM16LE(ppResult.audio)
                    try await outbound.write(.text(formatJSON([
                ...
                if let dec = state.spmDecoder, !ppResult.textTokens.isEmpty {
                    agentTranscript = dec.decode(ppResult.textTokens)
                }
                ...
                if !agentTranscript.isEmpty {
                    try await outbound.write(.text(formatJSON([
                        "type": "response.agent_transcript.done",
                        "response_id": responseId,
                ...
                let asrModel = try await state.loadASR()
                let text = asrModel.transcribe(audio: audio16k, sampleRate: 16000)
